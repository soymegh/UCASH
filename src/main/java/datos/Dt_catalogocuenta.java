...
public class Dt_catalogocuenta {
	...
	public void llenar_rsCatalogo(Connection c) {
		try {
			ps = c.prepareStatement("SELECT * FROM dbucash.catalogocuenta;", ResultSet.TYPE_SCROLL_SENSITIVE,
	...
	public ArrayList<Vw_catalogocuenta_empresa> listarCatalogocuenta() {
		...
		try {
			...
			ps = c.prepareStatement("SELECT * FROM dbucash.vw_catalogocuenta_empresa;", ResultSet.TYPE_SCROLL_SENSITIVE,
	...
	/**
	 * Lista los catálogos de cuenta de una empresa en específico
	 * @param idEmpresa
	 * @return ArrayList con los catálogos asociados a una empresa
	 */
	...
	// sujeto a cambios por nombres de entidades de la view registrada en la base de
	// datos
