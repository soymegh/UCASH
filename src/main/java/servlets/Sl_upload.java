...
import datos.Dt_tasacambio_det;
...
public class Sl_upload extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) 
		...
		Dt_tasaCambio dtsc = new Dt_tasaCambio();
		Dt_tasacambio_det dtscd = new Dt_tasacambio_det();
		...
		try {
			...
			if(!opt.equals("2")) {
				...
				var listcd = new ArrayList<Tbl_tasaCambioDet>();				
				var tcd = new Tbl_tasaCambioDet();				
				...
				if(idTasac>0){
					for(Tbl_tasaCambioDet tscd: listcd) {
					...
					//response.sendRedirect("upload-file-xlsx.jsp?msj=1");
						System.out.print("Guardado o eso espero  ");
					}
					else {
